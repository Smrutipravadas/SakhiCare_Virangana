...
class _BurnoutInsightScreenState extends State<BurnoutInsightScreen>
  ...
  Future<void> fetchInsights() async {
    try {
      ...
      setState(() {
        burnoutLevel = data["burnout_level"] ?? "No data";
        burnoutSuggestion = data["burnout_suggestion"] ?? "";

        avgSleep = (data["avg_sleep"] ?? 0).toDouble();
        avgWork = (data["avg_work"] ?? 0).toDouble(); // ✅ FIXED
        avgMood = (data["avg_mood"] ?? 0).toDouble();

        alertCareCircle = data["alert_care_circle"] ?? false;
        loading = false;
      });
